...
async def swap_faces(target: UploadFile = File(...), source: UploadFile = File(...)):
    try:
        ...
        print(f"Images encoded: source length={len(source_base64)}, target length={len(target_base64)}")  # Debug

        # Prepare payload for Gradio API
        payload = {
            "data": [source_base64, target_base64]  # [source_face, target_image]
        }
        ...
        response = requests.post(
            ...
            timeout=120  # Increased timeout for inference
        )

        print(f"HF API status: {response.status_code}")  # Debug

        if response.status_code == 200:
            result = response.json()
            if "data" in result and len(result["data"]) > 0:
                output_base64 = result["data"][0]
